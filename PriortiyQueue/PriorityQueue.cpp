...
//Heapify Taking in a vector as a paramater, O(n) to create
PriorityQueue :: PriorityQueue(vector<int> vals){       //
    this->data_ = vals;
}
...
void PriorityQueue :: enqueue(int val){
    ...
    if( this->type_ == greater){    //max heap
        while( parent ){//If there's a valid parent to compare with
...
void PriorityQueue :: pop(){ //Edge cases
    ...
    while( j < size_){  //Keep looping till we have a valid left child to compare to, remeber that size_ marks the boundary of the end of the vector
        ...
        if( i != priority){ //If we've seen a canidate to swap (ie a value greater or smaller) swap it
            swap(i, priority);
            i = priority;
            j = (i*2) +1;
